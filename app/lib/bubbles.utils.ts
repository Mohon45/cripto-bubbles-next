...
export class BubblesUtils {
	...
	static update = (circles: Circle[], imageSprites: PIXI.Sprite[], textSprites: PIXI.Text[], text2Sprites: PIXI.Text[], circleGraphics: PIXI.Sprite[] = []) => {
		return () => {
			for (let i = 0; i < circles.length; i++) {
				...
				//----------mouse drag effect-------------------
				if (circle.dragging) {
					circle.x = circle.dragData!.global.x;
					circle.y = circle.dragData!.global.y;
				} else {
					circle.x += circle.vx;
					circle.y += circle.vy;
				}
				...
				if (circle.radius !== circle.targetRadius) {
					...
					if (Math.abs(sizeDifference) <= changeSizeStep) {
						...
						//----------mouse drag effect------------------- disable bellow line
	...
	static handleEmptySpaceClick = (event: MouseEvent, circles: Circle[]) => {
		const waveForce = 25; // Adjust the wave force as needed
	...
	static handleMouseMove = (event: MouseEvent, circles: Circle[]) => {
		...
		if (index !== -1) {
			...
			const distance = Math.sqrt(dx * dx + dy * dy);
			const speed = 0.05; // Adjust the speed factor as needed
			const damping = 0.1;
			circle.vx = (dx / distance) * speed - circle.vx * damping;
			circle.vy = (dy / distance) * speed - circle.vy * damping;
		}
	};
	...
	// 	return shapes;
	// };
	static generateCircles = (coins: CoingeckoCoinData[], scalingFactor: number, bubbleSort: PriceChangePercentage = PriceChangePercentage.HOUR, page: number = 1, pageSize: number = 20): Circle[] => {
		// Calculate the start and end index for the current page
		const startIndex = (page - 1) * pageSize;
		const endIndex = startIndex + pageSize;

		// Slice the coins data to get only the bubbles for the current page
		const paginatedCoins = coins.slice(startIndex, endIndex);

		const shapes: Circle[] = paginatedCoins.map((item) => {
			const radius = Math.abs(item[bubbleSort]! * scalingFactor);
