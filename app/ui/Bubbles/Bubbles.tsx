...
export default function Bubbles({ coins = [] }: Props) {
	...
	useEffect(() => {
		...
		for (let i = 0; i < circles.length; i++) {
			...
			border.lineStyle(1, 0xffffff); // 2px white border
			border.drawCircle(0, 0, circle.radius + 2); // Slightly larger radius for the border
			border.visible = false; // Initially hide the border
			...
			container
				.on("pointerover", () => {
					border.visible = true;
					container.cursor = "pointer"; // Change cursor to pointer
				})
				.on("pointerout", () => {
					border.visible = false;
					container.cursor = "default"; // Revert cursor
				});
			...
			container
				.on("pointerdown", (event: any) => {
					...
					circle.dragData = event.data;
				})
				.on("pointerup", () => {
					circle.dragging = false;
					circle.dragData = null;
				})
				.on("pointerupoutside", () => {
					circle.dragging = false;
					circle.dragData = null;
				...
				.on("pointermove", () => {
					if (circle.dragging) {
						const newPosition = circle.dragData!.getLocalPosition(container.parent);
		...
		app.stage.on("pointermove", (event: any) => {
			BubblesUtils.handleMouseMove(event as PIXI.InteractionEvent, circles);
		});
		return () => {
	...
	return (
		<div className="flex rounded px-2 overflow-hidden bg-zinc-900 md:flex-col flex-col-reverse">
			...
			<div style={{ height: "90vh" }} className="bg-zinc-900 w-full overflow-hidden border-2 rounded border-gray-800" ref={appRef}></div>
			{isLoading && <Loader />}
		</div>
	);
}
